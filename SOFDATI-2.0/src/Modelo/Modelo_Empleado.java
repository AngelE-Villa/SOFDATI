...
public class Modelo_Empleado extends Empleado {
    ...
    public Modelo_Empleado(String cod_empleado, Double sueldo, String cargo, String password, String palabra, String cedula, String nombre, String apellido, Date fnacimineto, String direccion, String telefono) {
        super(cod_empleado, sueldo, cargo, password, palabra, cedula, nombre, apellido, fnacimineto, direccion, telefono);
    }
    ...
    public boolean buscarEmpleado(String cod){
        String query = "select cod_empleado from empleado where cedula_pe='"+cod+"'";
        ResultSet rs = con.query(query);
        try {
            while (rs.next()) {
                return true;
    ...
    public boolean ValidarDatos() {
        String sql = "SELECT * FROM empleado WHERE cedula_pe ='" + getCedula() + "' AND password='" + getPassword() + "'";
    ...
    public boolean Grabar_Empleado() {
        ...
        sql += "INSERT INTO empleado (cod_empleado,foto,cargo,sueldo,cedula_pe,password,palabra) "
                + "VALUES ('" + getCod_empleado()+ "','" + foto64() + "','" + getCargo()+ "'," + getSueldo() + ",'" + getCedula() + "','" + getPassword() + "','" + getPalabra() + "');";
    ...
    public List<Empleado> BuscarporID(String id) {
        try {
            String query = "select e.cod_empleado,e.foto,e.cargo,e.sueldo,p.cedula,p.nombre,p.apellido,p.fechanacimiento,p.direccion,p.telefono, e.password,e.palabra" +
            " from empleado e join persona p on p.cedula=e.cedula_pe where ";
            query += "UPPER(e.cod_empleado) LIKE UPPER('%" + id + "%') OR UPPER(e.cedula_pe) LIKE UPPER('%" + id + "%')";
            ResultSet rs = con.query(query);
            List<Empleado> lista = new ArrayList<Empleado>();
            byte[] bf;
            while (rs.next()) {
